...
- `find -exec`, `-execdir`, `-ok` and `-okdir` are checked: the command they run is held to the same allowlist, so `find . -exec rm {} \;` passes only if `rm` has an entry. Every argument to `find` must be literal. `find -delete` is not checked.
...
- The most dangerous invocation you can build from each entry you consider unsafe, for example `{{bin}} --check 'find . -name x -delete'` for `find`, or `{{bin}} --check 'git -c core.pager=touch\ pwned log'` for a bare `git`. If it passes, the entry really does allow it.
...
Only after the user agrees, edit `.bashcage.json`, then run `{{bin}} --list` and show the resulting allowlist.
